...
| **Librarian** | **Extension** (via Bridge) | Human Identity, Role, Agent Binding | ❌ Forbidden | Resolved identity must remain internal to The Librarian; only client/custody/status data may reach the extension |
...
- **No Human Identity Exposure:** Resolved human identity, role assignments, and agent bindings must remain internal to The Librarian. The Chrome extension must never receive, display, log, or persist resolved human identity fields. Only client/custody/status data may be returned to the extension.
